...
import { type BSData, type BSNode, type BSSummaryItem, formatAmount } from '../../api/Accounting/Balancesheet.api';
import { useBalanceSheet } from '../../hooks/Accounting/Balancesheet.logic';

/* ───────────────── KPI strip ───────────────── */

function KpiStrip({ summary, loading }: { summary: BSSummaryItem[]; loading: boolean }) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
      {items.map((item: any, i) => (
        <div key={item?.label ?? i} className="bg-white border border-gray-200 rounded-lg px-4 py-3 flex flex-col gap-1.5">
          ...
          {loading || !item ? (
            ...
            <span className={`text-sm font-bold tabular-nums ${colorFor(item)}`}>
...
function FilterBar({
  ...
  const { filters, setMode, setPeriodicity, setFromFiscalYear, setToFiscalYear, setFromDate, setToDate, handleRefresh, loading, allExpanded, handleToggleExpand } = bs;
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg px-4 py-3 flex flex-wrap items-end gap-3">
      <div className="flex flex-col gap-1">
        <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Mode</label>
        <select className={selectClass} value={filters.mode} onChange={(e) => setMode(e.target.value as any)}>
      ...
      <div className="flex flex-col gap-1">
        <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Period</label>
        <select className={selectClass} value={filters.periodicity} onChange={(e) => setPeriodicity(e.target.value as any)}>
      ...
      {filters.mode === 'Fiscal Year' ? (
        <>
          <div className="flex flex-col gap-1">
            <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">From FY</label>
            <input
              type="number"
              className={`${inputClass} w-24`}
              value={filters.fromFiscalYear}
              onChange={(e) => setFromFiscalYear(Number(e.target.value))}
            />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">To FY</label>
            <input
              type="number"
              className={`${inputClass} w-24`}
              value={filters.toFiscalYear}
              onChange={(e) => setToFiscalYear(Number(e.target.value))}
            />
          </div>
        </>
      ) : (
        <>
          <div className="flex flex-col gap-1">
            ...
            <input type="date" className={`${inputClass} w-[150px]`} value={filters.fromDate} onChange={(e) => setFromDate(e.target.value)} />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">To</label>
            <input type="date" className={`${inputClass} w-[150px]`} value={filters.toDate} onChange={(e) => setToDate(e.target.value)} />
          </div>
        </>
      )}
        ...
        <table className="border-collapse" style={{ tableLayout: 'fixed', width: 'max-content', minWidth: '100%' }}>
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={columns.length} className="py-10 text-center text-xs text-gray-400">
                  {emptyMessage}
                </td>
              </tr>
            ) : (
...
export function BalanceSheet() {
  ...
  const columns = useMemo<ColumnDef<BSNode>[]>(() => {
    ...
    return data.columns.map((col): ColumnDef<BSNode> => {
      if (col.fieldname === 'account') {
        return {
          ...
          cell: ({ row }) => {
            ...
            return (
              <div className="flex items-center gap-1.5" style={{ paddingLeft: `${row.depth * 18}px` }}>
                {canExpand ? (
                  <button type="button" onClick={row.getToggleExpandedHandler()} className="shrink-0 text-gray-400 hover:text-gray-700 flex items-center gap-1">
                    <IconChevronRight size={13} className={`transition-transform duration-150 ${row.getIsExpanded() ? 'rotate-90' : ''}`} />
                    {row.getIsExpanded() ? <IconFolderOpen size={14} /> : <IconFolder size={14} />}
                  </button>
                ) : (
                  <IconFileText size={13} className="text-gray-300 shrink-0" />
                )}
                <span className={`text-xs truncate ${node.is_group ? 'font-bold text-gray-900' : 'text-gray-700'}`}>{node.account_name}</span>
              </div>
            );
          },
        };
      ...
      return {
        ...
        meta: { align: 'right' },
        cell: ({ row }) => (
          <span className={`text-xs tabular-nums ${row.original.is_group ? 'font-bold text-gray-900' : 'text-gray-700'}`}>
  ...
  if (error && !data) {
    return (
      <div className="flex flex-col items-center py-20 gap-3">
        ...
        <button onClick={bs.handleRefresh} className="flex items-center gap-2 px-4 py-2 bg-[#1E3A8A] text-white text-sm font-semibold rounded-md hover:bg-[#1E3A8A]/90">
          <IconRefresh size={14} />
          Retry
        </button>
      </div>
  ...
  <p className="text-sm text-gray-500">
    Review assets, liabilities, equity, and your organization's overall financial position.
  </p>
</div>
      ...
      <div className="flex flex-col gap-2">
        <SectionHeader label="Application of Funds (Assets)" accentClass="bg-blue-600" />
        <BSTreeTable
          data={data?.assets ?? []}
          columns={columns}
      ...
      <div className="flex flex-col gap-2">
        <SectionHeader label="Source of Funds (Liabilities)" accentClass="bg-red-500" />
        <BSTreeTable
