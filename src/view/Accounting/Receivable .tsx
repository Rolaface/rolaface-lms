...
function KpiStrip({
  ...
  const sections = [
    ...
    },
    {
      ...
      label: 'Customers',
      items: [
        { label: 'Count', value: String(kpis.total_customers), color: 'text-indigoAlt-6', bold: true },
        { label: 'Paid', value: fmt(kpis.total_paid), color: 'text-emerald-600' },
        { label: 'Avg Days', value: String(kpis.average_collection_days || '—'), color: 'text-indigoAlt-6' },
      ],
    },
    {
...
export function Receivable() {
  const {
    ...
    selectedReceivableAccount, setSelectedReceivableAccount,
  ...
  const columns = useMemo<ColumnDef<ReceivableRow>[]>(
    () => [
      ...
      },
      {
        ...
        header: 'Customer',
        cell: ({ row }) => <span className="text-xs text-gray-800">{row.original.customer}</span>,
      },
      {
        id: 'voucherType',
