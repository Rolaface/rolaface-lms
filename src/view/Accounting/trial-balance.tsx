/* ───────────────────────────────────────────────────────────
   TrialBalance — UI layer
   All rendering: the filter bar, column defs, the tree table
   with sticky totals footer, and the page component wiring it
   all to useTrialBalance(). No state or business logic here.
   ─────────────────────────────────────────────────────────── */
...
import { type TBAccount, type TBFilters, nf } from '../../api/Accounting/Trialbalance.api';
import { useTrialBalance } from '../../hooks/Accounting/Trialbalance.logic';

/* ───────────────── Filter bar ───────────────── */

function FilterBar({
  filters, setFilters, onRefresh, loading,
}: {
  filters: TBFilters;
  ...
  return (
    <Paper withBorder radius="md" p="xs" className="shadow-sm">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <Group gap={4} wrap="nowrap">
          <Text fz="xs" fw={600} c="gray.5" className="uppercase tracking-wide">FY</Text>
          <TextInput
            size="xs"
            placeholder="Year"
            ...
            onChange={(e) => {
              ...
              if (/^\d{0,4}$/.test(value)) setFilters((f) => ({ ...f, fiscal_year: value }));
            }}
            className="w-20"
          />
        </Group>
        ...
        <Checkbox
          ...
          onChange={(e) => setFilters((f) => ({ ...f, show_zero_values: e.currentTarget.checked }))}
        />
        <Checkbox
          size="xs"
          label="Period Closing"
          checked={filters.with_period_closing_entry}
          onChange={(e) => setFilters((f) => ({ ...f, with_period_closing_entry: e.currentTarget.checked }))}
        />
        <Checkbox
          size="xs"
          label="Closing Entries"
          checked={filters.show_closing_entries}
          onChange={(e) => setFilters((f) => ({ ...f, show_closing_entries: e.currentTarget.checked }))}
        />
...
function useColumns(): ColumnDef<TBAccount>[] {
  return useMemo<ColumnDef<TBAccount>[]>(
    () => [
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex items-center gap-1.5" style={{ paddingLeft: row.depth * 18 }}>
              {canExpand ? (
                <ActionIcon size="xs" variant="subtle" color="gray" onClick={row.getToggleExpandedHandler()}>
                  <IconChevronRight
                    size={12}
              ...
              <Text fz="xs" fw={row.depth === 0 ? 600 : 400} c="gray.8" truncate>
                {node.account_name}
              </Text>
            </div>
          );
      ...
      {
        id: 'opening_debit',
        header: () => <Text fz="xs" fw={600} ta="right" w="100%">Opening Debit</Text>,
        size: 130,
        cell: ({ row }) => (
          <Text fz="xs" ta="right" c="gray.7" className="font-mono tabular-nums">{nf(row.original.opening_debit)}</Text>
        ),
      },
      {
        id: 'opening_credit',
        header: () => <Text fz="xs" fw={600} ta="right" w="100%">Opening Credit</Text>,
        size: 130,
        cell: ({ row }) => (
          <Text fz="xs" ta="right" c="gray.7" className="font-mono tabular-nums">{nf(row.original.opening_credit)}</Text>
        ),
      },
      {
        id: 'debit',
        header: () => <Text fz="xs" fw={600} ta="right" w="100%">Debit</Text>,
        size: 130,
        cell: ({ row }) => (
          <Text fz="xs" ta="right" fw={500} c="blue.6" className="font-mono tabular-nums">{nf(row.original.debit)}</Text>
        ),
      },
      {
        id: 'credit',
        header: () => <Text fz="xs" fw={600} ta="right" w="100%">Credit</Text>,
        size: 130,
        cell: ({ row }) => (
          <Text fz="xs" ta="right" fw={500} c="orange.6" className="font-mono tabular-nums">{nf(row.original.credit)}</Text>
        ),
      },
      {
        id: 'closing_debit',
        header: () => <Text fz="xs" fw={600} ta="right" w="100%">Closing Debit</Text>,
        size: 130,
        cell: ({ row }) => (
          <Text fz="xs" ta="right" fw={700} c="gray.9" className="font-mono tabular-nums">{nf(row.original.closing_debit)}</Text>
        ),
      },
      {
        id: 'closing_credit',
        header: () => <Text fz="xs" fw={600} ta="right" w="100%">Closing Credit</Text>,
        size: 130,
        cell: ({ row }) => (
          <Text fz="xs" ta="right" fw={700} c="gray.9" className="font-mono tabular-nums">{nf(row.original.closing_credit)}</Text>
        ),
      },
    ],
    [],
...
export function TrialBalance() {
  const {
    data, loading, error, handleRefresh,
    filters, setFilters,
    tableData, expanded, setExpanded,
  } = useTrialBalance();
              ...
              <tfoot className="sticky bottom-0 z-10">
                <tr className="bg-gray-50 border-t-2 border-gray-300 h-[34px] shadow-[0_-1px_0_0_rgba(0,0,0,0.06)]">
                  <td className="px-3 py-1 whitespace-nowrap bg-gray-50">
                    <Text fz="xs" fw={700} c="indigoAlt.6">TOTAL</Text>
                  </td>
                  <td className="px-3 py-1 whitespace-nowrap bg-gray-50 text-right">
                    <Text fz="xs" fw={700} c="gray.8" className="font-mono">{nf(data.totals.opening_debit)}</Text>
                  </td>
                  <td className="px-3 py-1 whitespace-nowrap bg-gray-50 text-right">
                    <Text fz="xs" fw={700} c="gray.8" className="font-mono">{nf(data.totals.opening_credit)}</Text>
                  </td>
                  <td className="px-3 py-1 whitespace-nowrap bg-gray-50 text-right">
                    <Text fz="xs" fw={700} c="blue.6" className="font-mono">{nf(data.totals.debit)}</Text>
                  </td>
                  <td className="px-3 py-1 whitespace-nowrap bg-gray-50 text-right">
                    <Text fz="xs" fw={700} c="orange.6" className="font-mono">{nf(data.totals.credit)}</Text>
                  </td>
                  <td className="px-3 py-1 whitespace-nowrap bg-gray-50 text-right">
                    <Text fz="xs" fw={700} c="gray.9" className="font-mono">{nf(data.totals.closing_debit)}</Text>
                  </td>
                  <td className="px-3 py-1 whitespace-nowrap bg-gray-50 text-right">
                    <Text fz="xs" fw={700} c="gray.9" className="font-mono">{nf(data.totals.closing_credit)}</Text>
                  </td>
                </tr>
              </tfoot>
            )}
