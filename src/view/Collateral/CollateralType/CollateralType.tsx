...
import {
  getAllLoanSecurityTypes,
  enableLoanSecurityType,
  disableLoanSecurityType,
  deleteLoanSecurityType
} from '../../../api/Collateral/loanSecurityTypeApi';
...
export function CollateralType() {
  ...
  const { data: typesResponse, isLoading } = useQuery({
    ...
    queryFn: () => getAllLoanSecurityTypes({ page_size: 1000 }), // Fetching all for client-side pagination
  });

  // Delete Mutation
  const { mutate: removeType, isPending: isDeleting } = useMutation({
    mutationFn: (id: string) => deleteLoanSecurityType(id),
    onSuccess: () => {
  ...
  const { mutate: toggleStatus } = useMutation({
    mutationFn: ({ id, disableFlag }: { id: string; disableFlag: number }) =>
      disableFlag === 1 ? disableLoanSecurityType(id) : enableLoanSecurityType(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['collateralTypes'] });
    },
  });
  ...
  // Map API response
  const data = useMemo(() => {
    if (typesResponse?.status_code === 200 && typesResponse?.status === 'success' && typesResponse.data) {
      return typesResponse.data.map((item: any) => ({
        id: item.name,
  ...
  const columns = useMemo(
                ...
                <ActionIcon
                  ...
                  onClick={() => {
                    modals.openConfirmModal({
                      ...
                      children: (
                        <Text size="sm">
                          Are you sure you want to delete the <b>{row.type}</b> collateral type? This cannot be undone.
                        </Text>
                      ),
                      labels: { confirm: 'Delete', cancel: 'Cancel' },
                      confirmProps: { color: 'red' },
                      onConfirm: () => removeType(row.id),
                    });
                  }}
                >
                  <IconTrash size={14} />
    ...
    [open, removeType, toggleStatus, isDeleting]
  );
  ...
  return (
    <Box className="flex flex-col gap-4 p-8 mt-10">
      <CollateralTypeModal 
        ...
        typeId={selectedTypeId} // Assuming you will update the Modal to accept these props
        isViewMode={isViewMode}
        onSuccess={() => queryClient.invalidateQueries({ queryKey: ['collateralTypes'] })} 
      />
