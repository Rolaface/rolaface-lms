...
import {
  ...
  IconArrowDown, IconFileSpreadsheet
...
export function LoanStatement() {
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('reference_no', {
        header: 'Reference No.',
        enableSorting: false,
        cell: (info) => <div className="text-slate-400 font-mono text-[11.5px]">{info.getValue()}</div>,
      }),
      columnHelper.accessor('transaction_type', {
        ...
        cell: (info) => {
          ...
          return (
            <Badge radius="sm" size="sm" style={{ backgroundColor: b.bg, color: b.color, textTransform: 'none' }}>
              {type}
            </Badge>
          );
        },
  ...
  const cashFlow = dashboardData?.cash_flow?.map((c: any) => ({
    month: c.month, Disbursal: c.disbursal, Repayment: c.repayment, "Charges/Interest": c.charges
  })) || [];

  const currentPct = dashboardData?.aging_summary?.find(a => a.label === "Current")?.percentage || 0;
  const agingPie = [
      ...
      <Box component="main" className="p-4 flex flex-col gap-3.5">
        ...
        <Paper withBorder radius="lg" p="sm" className="border-slate-200">
          <div className="flex flex-wrap gap-12">
            <Select 
              label="Customer" withAsterisk placeholder="Select customer" 
              data={["CUST-000123 - Rahul Sharma"]} defaultValue="CUST-000123 - Rahul Sharma" 
              classNames={inputClassNames} className="w-[280px]" 
              rightSection={<IconChevronDown size={13} className="text-slate-400" />} 
            />
            <Select 
              label="Loan Account" withAsterisk placeholder="Select account" 
              data={["LN-000456", "LN-000457"]} 
              value={filters.loanId} onChange={(val) => val && filters.setLoanId(val)}
              classNames={inputClassNames} className="w-[230px]" 
              rightSection={<IconChevronDown size={13} className="text-slate-400" />} 
            />
            <TextInput 
              label="From Date" withAsterisk type="date" 
              value={filters.fromDate} onChange={(e) => filters.setFromDate(e.currentTarget.value)}
              classNames={inputClassNames} className="w-[180px]" 
              rightSection={<IconCalendar size={14} className="text-slate-400" />} 
            />
            <TextInput 
              label="To Date" withAsterisk type="date" 
              value={filters.toDate} onChange={(e) => filters.setToDate(e.currentTarget.value)}
              classNames={inputClassNames} className="w-[180px]" 
              rightSection={<IconCalendar size={14} className="text-slate-400" />} 
            />
          </div>
        ...
        <div>
          ...
          <Group gap="sm" wrap="nowrap" className="overflow-x-auto relative min-h-[90px]">
            ...
            {[
              { label: "Opening Balance", value: formatCurrency(dashboardData?.summary?.opening_balance), icon: IconWallet, color: "indigoAlt" },
              { label: "Total Disbursed", value: formatCurrency(dashboardData?.summary?.total_disbursed), icon: IconArrowDown, color: "accent" },
              { label: "Total Repayments", value: formatCurrency(dashboardData?.summary?.total_repayments), icon: IconArrowUp, color: "brand" },
              { label: "Total Charges", value: formatCurrency(dashboardData?.summary?.total_charges), icon: IconReceipt2, color: "gold" },
              { label: "Closing Balance", value: formatCurrency(dashboardData?.summary?.closing_balance), icon: IconWallet, color: "brand", highlight: true },
            ].map((c) => <SummaryCard key={c.label} card={c} />)}
          </Group>
        </div>
          ...
          <ChartCard title="Loan Snapshot">
            <div className="flex flex-col gap-1.5 overflow-y-auto h-full pr-1">
              {[
                ...
                ["Loan Amount", formatCurrency(dashboardData?.snapshot?.loan_amount)],
                ["Disbursed Amount", formatCurrency(dashboardData?.snapshot?.disbursed_amount)],
                ["ROI (%)", `${dashboardData?.snapshot?.roi || 0}%`],
                ["EMI Amount", formatCurrency(dashboardData?.snapshot?.emi_amount)],
              ...
              <div className="w-full flex flex-col gap-1.5 mt-3">
                {dashboardData?.aging_summary?.map((a, i) => (
                  <Group key={a.label} justify="space-between">
                    ...
                    <Text size="11px" fw={500} className="text-slate-700">{a.percentage}% ({formatCurrency(a.amount)})</Text>
                  </Group>
                ))}
              </div>
              <Text size="10px" c="dimmed" mt={8} className="self-start opacity-70">* DPD as on today</Text>
            </div>
          </ChartCard>
        </div>
        ...
        <Paper withBorder radius="lg" className="border-slate-200 overflow-hidden">
          ...
          <div className="overflow-x-auto min-h-[250px] relative">
            {status.loadingTable ? (
              <Group justify="center" align="center" className="absolute inset-0 z-10 bg-white/70">
          ...
          <Group justify="space-between" p="sm" className="border-t border-slate-100">
            <Text size="12px" c="dimmed">
              Showing {totalRows === 0 ? 0 : firstRow} to {lastRow} of {totalRows} entries
            </Text>
            <Group gap={12}>
              <Pagination 
                total={totalPages} 
