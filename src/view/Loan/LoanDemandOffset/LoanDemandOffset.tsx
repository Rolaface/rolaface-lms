...
export function LoanDemandOffset() {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((o) => {
      const matchesSearch =
        !q || o.name.toLowerCase().includes(q) || o.components.toLowerCase().includes(q);
      const matchesStatus =
        status === 'all' ||
        (status === 'active' && o.status === 'ACTIVE') ||
        (status === 'inactive' && o.status === 'INACTIVE');
  ...
  return (
    <Box className="flex flex-col gap-4">
      {/* If you have a Modal, insert it here:
          <LoanDemandOffsetModal opened={opened} onClose={close} /> 
      */}
      ...
      <Paper withBorder radius="md" p="xs" className="shadow-sm">
        <div className="flex items-center gap-3 flex-wrap">
          <TextInput
            size="xs"
            placeholder="Search Offset Name / Component"
