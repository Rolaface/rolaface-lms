  ...
  return (
    <div className="flex flex-col gap-1 border-b border-[var(--mantine-color-slate-1)] py-2.5 pr-4">
      <div className="flex items-center gap-1.5">
        {icon && (
          <span className="shrink-0" style={{ color: 'var(--mantine-color-slate-4)' }}>
            {icon}
          </span>
        )}
        <Text size="xs" c="slate.5" fw={500} className="uppercase tracking-wide">
          {label}
        </Text>
      </div>
      <Text size={emphasis ? 'sm' : 'xs'} fw={emphasis ? 700 : 600} c={isEmpty ? 'slate.4' : 'slate.8'}>
        {isEmpty ? '—' : value}
      </Text>
    </div>
  );
...
export function FieldGrid({
  ...
  entries: { label: string; value: React.ReactNode; icon?: React.ReactNode; emphasis?: boolean }[];
  columns?: 2 | 3 | 4;
}) {
  const colClass =
    columns === 4
  ...
  return <div className={`grid grid-cols-1 ${colClass}`}>{entries.map((entry) => <DataField key={entry.label} {...entry} />)}</div>;
}
  ...
  return (
    <div
      className={`flex items-center justify-between gap-4 py-2 ${
        bordered ? 'border-b border-[var(--mantine-color-slate-1)] last:border-b-0' : ''
      }`}
    >
      <div className="flex items-center gap-2" style={{ color: 'var(--mantine-color-slate-5)' }}>
        {icon}
  ...
  return (
    <Paper withBorder radius="md" p="md" style={{ borderColor: 'var(--mantine-color-slate-2)' }}>
      <div className="flex items-center justify-between">
        ...
        {icon && (
          <ThemeIcon variant="light" color={isEmpty ? 'gray' : tone} size={26} radius="md">
            {icon}
          </ThemeIcon>
        )}
      </div>
...
export function FacilityCountTile({
  ...
  return (
    <div
      ...
      style={{ backgroundColor: 'var(--mantine-color-slate-0)', border: '1px solid var(--mantine-color-slate-2)' }}
    >
      {icon && (
...
export function StatusBadge({ status }: { status?: string }) {
  ...
  const color = ['verified', 'clear', 'complete', 'passed', 'low', 'uploaded', 'assessed', 'active'].includes(s)
    ? 'success'
...
export function CardLinkButton({ label, onClick }: { label: string; onClick?: () => void }) {
  return (
    <Button fullWidth variant="light" color="brand" size="xs" mt="md" rightSection={<IconArrowRight size={14} />} onClick={onClick}>
      {label}
    </Button>
  );
}
...
export function DocStatusRow({ name, meta, status }: { name: string; meta?: string; status?: string }) {
  const ok = ['verified', 'uploaded', 'complete'].includes((status ?? '').toLowerCase());
  ...
  return (
    <div className="flex items-center justify-between gap-3 border-b border-[var(--mantine-color-slate-1)] py-2 last:border-b-0">
      <div className="flex items-center gap-2 min-w-0">
        <ThemeIcon variant="light" color={ok ? 'success' : 'gray'} size={22} radius="xl">
