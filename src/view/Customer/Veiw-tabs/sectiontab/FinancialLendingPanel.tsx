    ...
    <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:items-center">
          <div className="shrink-0 mx-auto lg:mx-0">
            <CreditScoreGauge
              ...
              lastFetchedAt={
                credit.fetchedAt ? new Date(credit.fetchedAt) : null
              }
              ...
              entries={[
                {
                  ...
                  value: field(credit.activeFacilities),
                  icon: <IconLayoutGrid size={13} />,
                },
                { label: "Defaults", value: field(credit.defaults) },
                {
                  label: "Delinquencies",
                  value: credit.delinquencies
                    ? `${credit.delinquencies} flagged`
                    : field(credit.delinquencies),
                  icon: <IconFlag size={13} />,
                },
                {
                  label: "Recent Inquiries",
                  value: credit.recentInquiries
                    ? `${credit.recentInquiries} (90d)`
                    : field(credit.recentInquiries),
      ...
      <SectionCard icon={<IconWallet size={16} />} title="Facility Snapshot">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <FacilityCountTile
            label="Loans"
            count={b.loans?.length ?? 0}
            icon={<IconBuildingBank size={16} />}
          />
          <FacilityCountTile
            label="Investments"
            count={b.investments?.length ?? 0}
            icon={<IconChartArrows size={16} />}
          />
          <FacilityCountTile
            label="Savings"
            count={b.savings?.length ?? 0}
            icon={<IconWallet size={16} />}
          />
          <FacilityCountTile
            label="Fixed Deposits"
            count={b.fixedDeposits?.length ?? 0}
          ...
          entries={[
            {
              ...
              value: field(financial.educationLevel),
            },
            {
              label: "Employment Type",
              value: field(financial.employmentType),
            },
            {
              label: "Source of Income",
              value: field(financial.sourceOfIncome),
            },
            {
              label: "Monthly Income",
              value: financial.monthlyIncome
                ? money(financial.monthlyIncome)
                : "—",
            },
            {
              label: "Annual Income",
              value: financial.annualIncome
                ? money(financial.annualIncome)
                : "—",
            },
            {
            ...
            },
            {
              ...
              value: field(
                financial.relationshipManager ?? b.relationshipManager?.name,
              ),
            },
          ]}
        />
