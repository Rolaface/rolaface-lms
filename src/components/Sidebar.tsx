...
import { Box, Text, Stack, UnstyledButton, Avatar, ActionIcon, Collapse } from '@mantine/core';
import { 
  IconShieldCheck,
  IconLayoutDashboard,
  IconCalculator,
...
const LOCAL_NAV_ITEMS = [
  ...
  { 
    ...
    subItems: [
      ...
      { path: '/setup/classification', label: 'Loan Classification Codes', icon: IconFileText },
      { path: '/setup/provisioning', label: 'Provisioning', icon: IconCalculator },
  ...
  { 
    ...
    subItems: [
      ...
      { path: '/operations/prepayment', label: 'Loan Prepayment', icon: IconCalculator },
      ...
      { path: '/operations/settlement', label: 'Full Settlement', icon: IconShieldCheck },
...
export function Sidebar({ isCollapsed, onToggle }: { isCollapsed: boolean; onToggle: () => void }) {
  ...
  return (
    <Box className="flex h-full flex-col bg-white overflow-hidden">  
     ...
     {/* Navigation Items */}
      <Stack gap={16} className={`flex-1 py-8 overflow-y-auto ${isCollapsed ? 'px-4 items-center' : 'px-6'}`}>
        {LOCAL_NAV_ITEMS.map((item) => {
          ...
          return (
            <Box key={item.label} className="w-full">
              ...
              >
                <Box className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-4'}`}>
                  <ItemIcon
                    // size={22}
                    size={SIZES.rootIcon}
                    stroke={1.5}
                    className={isRootActive || (isOpen && hasSubItems && !isCollapsed) ? 'text-[#1E40AF]' : 'text-gray-500'}
                  />
                ...
                {!isCollapsed && hasSubItems && (
                  <Box className="text-gray-400">
                    {isOpen ? <IconChevronUp size={SIZES.chevron} className="text-[#1E40AF]" /> : <IconChevronDown size={16} />}
                  </Box>
                )}
              </UnstyledButton>
...
{hasSubItems && !isCollapsed && isOpen && (
  <Box className="ml-[11px] mt-2 mb-2 flex flex-col gap-3 border-l-2 border-gray-200 py-1 pl-6">
    {item.subItems!.map((subItem) => {
      ...
      return (
        <UnstyledButton
          ...
          className={`flex items-center gap-3 text-[14px] font-medium transition-colors ${
            isSubActive ? 'text-[#1E40AF]' : 'text-gray-600 hover:text-[#1E40AF]'
          }`}
        ...
        >
          <SubIcon
            ...
            size={SIZES.subIcon}
            stroke={1.5}
            className={isSubActive ? 'text-[#1E40AF]' : 'text-gray-500'}
          />
      ...
      {/* Footer: User Profile */}
      <Box className={`border-t border-gray-200 shrink-0 ${isCollapsed ? 'p-2 flex flex-col items-center gap-4' : 'p-4'}`}>
