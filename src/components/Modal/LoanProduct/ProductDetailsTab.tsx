    ...
    <div className="flex flex-col gap-4">
      <PlainCard>
        <div className="grid grid-cols-3 gap-x-6 gap-y-4">
          ...
          <NumberInput label="Maximum Loan Amount" withAsterisk min={0}
            allowNegative={false} decimalScale={2} hideControls
            {...form.getInputProps("maxLoanAmount")} />

          <NumberInput label="Days Past Due Threshold for NPA" withAsterisk min={0}
            allowNegative={false} allowDecimal={false} hideControls
            {...form.getInputProps("npaThreshold")} />
        </div>
      </PlainCard>
        ...
        <div className="grid grid-cols-5 gap-5">
          <div className="col-span-2 rounded-xl border p-4" style={{ backgroundColor: theme.indigoAlt[0], borderColor: theme.indigoAlt[1] }}>
            ...
            <div className="grid grid-cols-2 gap-x-5 gap-y-3">
              <NumberInput label="Interest Rate (%)" withAsterisk min={0} max={100}
                allowNegative={false} decimalScale={2} hideControls
          ...
          <div className="col-span-3 rounded-xl border p-4" style={{ backgroundColor: theme.danger[0], borderColor: theme.danger[1] }}>
            ...
            <div className="grid grid-cols-3 gap-x-5 gap-y-3">
              <NumberInput label="Penalty Rate (%)" withAsterisk min={0} max={100}
                allowNegative={false} decimalScale={2} hideControls
              ...
              <NumberInput label="Grace Period (Days)" withAsterisk min={0}
                allowNegative={false} allowDecimal={false} hideControls
                {...form.getInputProps("gracePeriodDays")} />
            </div>
          </div>
        </div>
      </SectionCard>
